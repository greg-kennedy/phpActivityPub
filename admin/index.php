...
// NW set mode to create db if not existing
$db = new SQLite3("db.sqlite3", SQLITE3_OPEN_READWRITE | SQLITE3_OPEN_CREATE);
...
// NW added profile table
query($db, 'CREATE TABLE IF NOT EXISTS profile (user STRING NOT NULL COLLATE NOCASE REFERENCES acct(user) ON DELETE CASCADE ON UPDATE CASCADE, summary TEXT, url TEXT, name TEXT, icon TEXT, homepage TEXT , PRIMARY KEY(user)) WITHOUT ROWID');
...
foreach ($actors as $a) {
  ...
  echo '<tr><td>', $a['user'], '</td><td>', $a['type'], '</td><td>', $a['key'], "</td><td><a href='profile.php?user=" . $a['user'] . "'>Profile</a></td></tr>\n";
}
?>
  </table>
  <hr>
